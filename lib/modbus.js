...
class Modbus extends events.EventEmitter {
  ...
  async pollRegistersByFC(functionCode = 4) {
    ...
    for (let key of keys) {
      ...
      try {
        if ( functionCode === 4 ) {
          data = await this.modbus.readInputRegisters(address, size * length / 2);  // length of the reading is in unit 'register', which is = 1 word
        }
        else {
          data = await this.modbus.readHoldingRegisters(address, size * length / 2); // length of the reading is in unit 'register', which is = 1 word
        }
      }
      catch (err) {
        ...
        logger.error('Error while reading register: ' + key + ' - ' + err);
        logger.info('Register description: ' + JSON.stringify(addressDescriptor, null, 2));
        continue;
      }
      ...
      switch ( type ) {
        ...
        case 'float':
          value = ( endian == 'little' ) ? data.buffer.readFloatLE() : data.buffer.readFloatBE();
        break;

        case 'float[]':
          value = [];
          for ( let i = 0; i < length; i++ ) {
            value.push(( endian == 'little' ) ? data.buffer.readFloatLE(i * size) : data.buffer.readFloatBE(i * size));
          }
        break;
        ...
        case 'int32':
          value = ( endian == 'little' ) ? data.buffer.readInt32LE() : data.buffer.readInt32BE();
        break;

        case 'int32[]':
          value = [];
          for ( let i = 0; i < length; i++ ) {
            value.push(( endian == 'little' ) ? data.buffer.readInt32LE(i * size) : data.buffer.readInt32BE(i * size));
          }
        break;

        case 'uint32':
          value = ( endian == 'little' ) ? data.buffer.readUInt32LE() : data.buffer.readUInt32BE();
        break;

        case 'uint32[]':
          value = [];
          for ( let i = 0; i < length; i++ ) {
            value.push( ( endian == 'little' ) ? data.buffer.readUInt32LE(i * size) : data.buffer.readUInt32BE(i * size) );
          }
        break;

        case 'int':
          value = ( endian == 'little' ) ? data.buffer.readInt16LE() : data.buffer.readInt16BE();
        break;

        case 'int[]':
          value = [];
          for ( let i = 0; i < length; i++ ) {
            value.push( ( endian == 'little' ) ? data.buffer.readInt16LE(i * size) : data.buffer.readInt16BE(i * size) );
          }
        break;
        ...
        case 'uint':
          value = ( endian == 'little' ) ? data.buffer.readUInt16LE() : data.buffer.readUInt16BE();
        break;

        case 'uint[]':
          value = [];
          for ( let i = 0; i < length; i++ ) {
            value.push( ( endian == 'little' ) ? data.buffer.readUInt16LE(i * size) : data.buffer.readUInt16BE(i * size) );
          }
        break;
